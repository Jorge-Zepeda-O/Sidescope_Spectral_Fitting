...
menu_file = uimenu(MainWin, 'Text', 'File');
	UI.MakeMenu(MainWin, menu_file, "Load Image", 'off', @menu_load_OnClick);
	
menu_plt = uimenu(MainWin, 'Text', 'Plotting');
	UI.MakeMenu(MainWin, menu_plt, "Use eVs", 'off', @menu_plot_OnClick, 1);
	UI.MakeMenu(MainWin, menu_plt, "Show BG Spectra", 'on', @menu_plot_OnClick, 2, 'on');
	UI.MakeMenu(MainWin, menu_plt, "Show Signal Spectrum", 'on', @menu_plot_OnClick, 3);
...
%% UI CONTROLS %%
% Buttons %
UI.MakeButton(MainWin, [0.05, 0.10, 0.10, 0.05], "Zoom", 'zoom', true, 'off');
UI.MakeButton(MainWin, [0.05, 0.05, 0.10, 0.05], "Pan", 'pan', true, 'off');

UI.MakeButton(MainWin, [0.20, 0.10, 0.15, 0.05], "Select Particles", ...
	@sel_part_OnClick, false, 'off');
UI.MakeButton(MainWin, [0.20, 0.05, 0.15, 0.05], "Fit Selection", ...
	@fit_part_OnClick, false, 'off');

UI.MakeButton(MainWin, [0.80, 0.10, 0.10, 0.05], "Export Data", ...
	@export_OnClick, false, 'off');
...
%		storing and keeping track of many different instance variables.
	...
	active = [parent.UserData.Frames(:).isActive];
	parent.UserData.Frames(active).DispImg;
	
	% Draw the Frame Slider if applicable %
	if(length(parent.UserData.Frames) > 1)
		UI.MakeFrameSlider(parent, [0.05, 0.25, 0.30, 0.05], "Frame Control", ...
			[1, 1, length(parent.UserData.Frames)], @ctrl_fsld_OnValueChanged);
	end
	
	% Enable the Zoom, Pan, and Select Particle Buttons %
	UI.EnableControl(parent, "btn: Zoom");
	UI.EnableControl(parent, "btn: Pan");
	UI.EnableControl(parent, "btn: Select Particles");
end
...
%	> numlbl:	(UIControl) The associated numeric label UIControl handle.
	...
	% Find the currently active frame %
	active = find([parent.UserData.Frames(:)], 1);
	
	% If there isn't a difference between 'val' and 'active' then do nothing %
	if(active == val), return; end
	
	% Else, deactivate the currently active frame and activate the new frame %
	parent.UserData.Frames(active).isActive = false;
	parent.UserData.Frames(val).isActive = true;

	%% MainWin Update %%
	% Get the active Frame (as we won't be modifying it any more) %
	activeFrame = parent.UserData.Frames(val);
	
	% Draw the new Frame in the 'Original Image' axes %
	activeFrame.DispImg(UI.axs(1));
	...
	for p = 1:lnum_part
		...
		activeFrame.Particles(p).DispBox(UI.axs(1), p.isActive);
		
		% In the meantime, find out which one of the Particles is the active one %
		if(p.isActive), active = p; end
	end
	% Send the particle strings to the 'Found Particles' Listbox %
	UI.SetControlParam(parent, "lbx: Found Particles", ...
		'String', [parent.UserData.Frames(val).Particles(:).str]);
	
	% Get the active Particle (as we also won't be modifying it any more) %
	activeParticle = activeFrame.Particles(active);
	
	% Display the peak and spectrum images of the active particle %
	activeParticle.DispPeak;
	activeParticle.DispSpec;
	
	% Display the spectrum plot of the active particle %
	activeParticle.DispPlot;
...
function sel_part_OnClick(parent, ~, ~)
	...
	UI.SetControlParam(parent, "lbx: Found Particles", 'Enable', 'on');
	
	% Update the Listbox title %
	UI.SetControlParam(parent, "ttllbx: Found Particles", 'String', ...
	...
	UI.EnableControl(parent, "btn: Fit Selection");
	UI.EnableControl(parent, "btn: Export Data", 'off');
end
