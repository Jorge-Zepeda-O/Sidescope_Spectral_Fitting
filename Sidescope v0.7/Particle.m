...
methods
	...
	%	
		...
		spec_fit = Fit(flip(Particle.RNG_EV), 3, true);
		spec_fit.Fit_Lorentzian(spec_sig, numfits);
		...
		for f = 1:numfits
			this.spec_fits(f) = Fit(flip(Particle.RNG_EV), 3, f == 1);
		end
		...
		if(numfits == 1)
			...
			for i = 1:Particle.fitval(4)
				...
				for f = 1:numfits
					...
					fit_other = sum([this.spec_fits([1:numfits] ~= f).curve], 2);

					% Get the error of fit_other with the raw data %				
					[issue, fit_err, fit_issue] = Particle.FitIssue(spec_sig, fit_other, ...
						sqrt(pow_noise) / Particle.fitval(2));
					if(~issue)
						if(~this.spec_fits(f).isActive)
							continue;
						else
							break;
						end
					...
					this.spec_fits(f).Fit_Lorentzian(fit_err);

					figure(2);
					plot(this.spec_fits(f).domain, [spec_sig - fit_other, fit_err.*fit_issue]);
					hold on;
					plot(this.spec_fits(f).domain, [this.spec_fits.curve], 'k', 'LineWidth', 2);
					plot(this.spec_fits(f).domain, this.spec_fits(f).curve, 'g', 'LineWidth', 2);
					hold off;
					legend("Error", "Issue", "Fits");
					drawnow limitrate;
					
					% If this fit doesn't really matter too much, nuke it %
					if(this.spec_fits(f).param(3)^2 < Particle.fitval(1)*pow_noise)
						this.spec_fits(f).isActive = false;
						this.spec_fits(f).Refresh();
					else
						this.spec_fits(f).isActive = true;
					end
				end
...
methods(Static)
	...
	%			(~) The default will reference the 'Peak Image' axes 
		...
		text(ax, size(part.spec_img, 1)*(0.10) + Frame.SBAR_PX/20, ...
			size(part.spec_img, 1)*(1-0.12), "1\mum", ...
	...
	%			(~) The default will reference the 'Spectrum Image' axes 
		...
		text(ax, size(part.spec_img, 2)*(0.05) + Frame.SBAR_PX/2, ...
			size(part.spec_img, 1)*(1-0.12), "10\mum", ...
			'Interpreter', 'tex', 'Color', 'w', 'HorizontalAlignment', 'center');
	end
	...
	%			(~) The default will reference the 'Spectrum Plot' axes 
		...
		if(Particle.pltopt(5))	% Show Fit Decomposition %
			plot(ax, xgrid, [part.spec_fits.curves], '-', 'Color', [0, 0.8, 0], ...
				'LineWidth', 2, 'HandleVisibility', 'off');
		end
		
		% Plot the Total Lorenzian Fit %
		plot(ax, xgrid, sum([part.spec_fits.curves], 2), 'k-', 'LineWidth', 2);
